...
export function RoleProvider({ children }: { children: React.ReactNode }) {
  ...
  const setRole = useCallback((nextRole: Role) => {
    ...
    const usesCompanyScope = !['dealer', 'system_admin'].includes(nextRole);
    writeApiSession({
      actorId: `demo-${nextRole}`,
      role: apiRole,
      companyId: usesCompanyScope ? current?.companyId ?? 1 : null,
      dealerId: nextRole === 'dealer' ? 1 : null,
      siteId:
        nextRole === 'site_manager' || nextRole === 'operator'
          ? current?.siteId ?? 1
          : null,
    });
    setUser(null);
